...
def get_opensearch(the_host="localhost", port=9200):
    host = the_host
    auth = ('admin', 'admin')
    client = OpenSearch(
...
def query_opensearch(worker_num, query_file: str, host: str, port: int, index_name: str, max_queries: int, seed: int, stop_event):
    ...
    client = get_opensearch(host, port)
    start = perf_counter()
    i = 0
    modulo = 1000
...
@click.option('--port', '-p', default=9200, help="The port of the OpenSearch")
...
def main(query_file: str, index_name: str, host: str, port: int, max_queries: int, seed: int, workers: int):
    ...
    with Manager() as manager:
        ...
        with concurrent.futures.ProcessPoolExecutor(max_workers=workers) as executor:
            futures = [executor.submit(query_opensearch, i, query_file, host, port, index_name, max_queries, seed*(i+1), stop_event) for i in range(workers)]
